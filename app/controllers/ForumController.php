...
class ForumController extends BaseController {
    ...
    public function storeGroup()
    {
        ...
        if($validator->fails()) {
            ...
            if($group->save()) {
                ...
                return Redirect::route('forum-home')->with('error', 'Unfortunately, an error occurred.');
            }
        }
    }
    ...
    public function storeCategory($id)
    {
        ...
        if($validator->fails()) {
            return Redirect::route('forum-home')->withInput()->withErrors($validator)->with('category-modal', '#category-modal')->with('group-id', $id);
        } else {
            $group = ForumGroup::find($id);
            if ($group == null) {
                return Redirect::route('forum-home')->with('fail', "That group doesn't exist.");
            ...
            if($category->save()) {
                ...
                return Redirect::route('forum-home')->with('error', 'An error occurred while saving the category.');
            }
        }
    }
    ...
    public function storeThread($id)
    {
        ...
        if ($validator->fails())
        {
            ...
            if ($thread->save()) {
                ...
                return Redirect::route('forum-get-new-thread', $id)->with('fail', "An error occurred while saving your thread.")->withInput();
            }
        }
    }
    ...
    public function storeComment($id)
    {
        ...
        if($thread == null) {
            Redirect::route('forum-new-comment')->with('fail', 'You posted to an invalid thread.');
        ...
        if ($validator->fails())
        {
            return Redirect::route('forum-new-comment', $id)->withInput()->withErrors($validator)->with('fail', "Your input doesn't match the requirements");
        } else {
            $comment = new ForumComment;
            $comment->title = Input::get('title');
            $comment->body = Input::get('body');
            $comment->category_id = $id;
            $comment->group_id = $comment->group_id;
            $comment->author_id = Auth::user()->id;

            if ($thread->save()) {
                return Redirect::route('forum-comment', $thread->id)->with('success', "Your comment has been saved.");
            } else {
                return Redirect::route('forum-new-comment', $id)->with('fail', "An error occurred while saving your comment.")->withInput();
            }
        }
    }
}